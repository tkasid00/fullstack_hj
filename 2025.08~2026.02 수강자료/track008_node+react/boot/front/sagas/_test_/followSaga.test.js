...
import { follow, unfollow, loadFollowers, loadFollowings, updateBlock, toggleFollow } from '../followSaga';
...
describe('followSaga CRUD', () => {
  ...
  it('follow success', async () => {
    const dispatched = [];
    axios.post.mockResolvedValue({ data: { followeeId: 2, blocked: false } });  //(2) 서버에서 결과값

    const action = followRequest({ followeeId: 2 });  //react -veiw(1)
  ...
  it('unfollow success', async () => {
    const dispatched = [];
    axios.delete.mockResolvedValue({ data: { followeeId: 2 } });

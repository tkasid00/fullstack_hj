...
desc food;  --사료라벨
desc foodbrand; --브랜드
desc foodnutrient; --라벨영양소
desc nutrient; --영양소단위
desc nutrientrange;  --영양소범위

select b.brandname, f.foodname, 
n.NUTRIENTNAME, fn.AMOUNT, n.unit
from foodbrand b join food f on(b.brandid=f.brandid) 
        join  foodnutrient fn on(f.foodid=fn.foodid) 
...
SELECT 
  b.brandname, 
  f.foodname,
  LISTAGG(n.NUTRIENTNAME ||  TO_CHAR(fn.AMOUNT, 'FM9990.00') || n.unit, ', ') 
    WITHIN GROUP (ORDER BY n.NUTRIENTNAME) AS nutrients
FROM foodbrand b 
JOIN food f ON b.brandid = f.brandid
JOIN foodnutrient fn ON f.foodid = fn.foodid
JOIN nutrient n ON fn.NUTRIENTID = n.NUTRIENTID
GROUP BY b.brandname, f.foodname;

commit;

select * from food order by foodid desc;